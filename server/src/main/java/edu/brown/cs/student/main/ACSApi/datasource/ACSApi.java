...
public class ACSApi implements CensusDatasource {
  ...
  public BroadbandData getBroadbandData(String state, String county) throws Exception {
      ...
      HttpURLConnection connection = RequestHelpers.tryPublicRequest(
      ...
      if (connection.getResponseCode() != 200){
        throw new Exception("Error from ACS API: " + new Buffer().readFrom(connection.getErrorStream()).readUtf8());
      }
      List<List<String>> response = JsonSerializer.fromJson(
  ...
  private void fetchStateCounties(int stateCode) throws Exception {
    HttpURLConnection connection = RequestHelpers.tryPublicRequest(
        "https://api.census.gov/data/2010/dec/sf1?get=NAME&for=county:*&in=state:" + String.format("%02d", stateCode));
    if (connection.getResponseCode() != 200){
      throw new Exception("request is failing to get county codes for " + stateCode + ": " +
          new Buffer().readFrom(connection.getErrorStream()).readUtf8());
    }
    countyCodes.put(stateCode, JsonSerializer.fromJson(
        new Buffer().readFrom(connection.getInputStream()).readUtf8(),
  ...
  private void fetchStateCodes() throws Exception {
    ...
    if (connection.getResponseCode() != 200){
      throw new Exception("request is failing to get state codes: " +
          new Buffer().readFrom(connection.getErrorStream()).readUtf8());
    }
    this.stateCodes = JsonSerializer.fromJson(
        new Buffer().readFrom(connection.getInputStream()).readUtf8(),
        Types.newParameterizedType(Map.class, String.class, Integer.class));
  }
}