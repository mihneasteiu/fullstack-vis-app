...
/**
 * A mock implementation of the CensusDatasource interface.
 * This class simulates the ACS (American Community Survey) API by reading data from a JSON file.
 */
public class MockACS implements CensusDatasource {
   ...
   * @throws IllegalArgumentException If the specified state and county combination is not found in the mock data.
  ...
  public BroadbandData getBroadbandData(String state, String county)
    ...
    Type ListString = Types.newParameterizedType(List.class,
    ...
    for (List<String> entry : data) {
      if (entry.size() == 3 && entry.get(1).equalsIgnoreCase(state) && entry.get(2).equalsIgnoreCase(county)) {
        return new BroadbandData(Float.parseFloat(entry.get(0)), state, county,
